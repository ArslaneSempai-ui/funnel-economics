 ...
 */
import { generate, SCENARIO, reached } from "./population.js";
...
const retained = (s) => generate(s).filter((u) => reached(u, "retain")).length;
/**
 ...
 */
export function price(step, points, s = SCENARIO, a = ASSUMPTIONS) {
    ...
    const perYear = (n) => (n * 12) / s.months;
    const extraRetained = perYear(after - before);
    return { extraRetained, extraRevenue: extraRetained * a.annualRevenuePerCustomer };
}
export function priceAll(s = SCENARIO, a = ASSUMPTIONS, levers = LEVERS) {
    const steps = Object.keys(levers);
...
export function compare(alternative, s = SCENARIO, a = ASSUMPTIONS) {
    const base = priceAll(s, a);
    const other = priceAll(s, a, { ...LEVERS, ...alternative });
    return {
        base, other,
...
export function verdict(priced) {
    const best = priced[0];
...
if (isMain(import.meta)) {
    ...
    for (const p of priced) {
        console.log(`${p.step.padEnd(11)}${("+" + (p.points * 100).toFixed(0) + " pt").padStart(7)}` +
